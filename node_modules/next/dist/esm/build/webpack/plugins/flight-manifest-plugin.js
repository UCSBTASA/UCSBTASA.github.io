...
export class ClientReferenceManifestPlugin {
    ...
    createAsset(assets, compilation, context) {
        ...
        compilation.chunkGroups.forEach((chunkGroup)=>{
            ...
            const groupName = entryName.slice(0, entryName.lastIndexOf("/")).replace(/\/@[^/]+/g, "").replace(/\/\([^/]+\)/g, "");
        ...
        for (const pageName of manifestEntryFiles){
            ...
            assets["server/app" + pageBundlePath + "_" + CLIENT_REFERENCE_MANIFEST + ".js"] = new sources.RawSource(`globalThis.__RSC_MANIFEST=(globalThis.__RSC_MANIFEST||{});globalThis.__RSC_MANIFEST[${JSON.stringify(pagePath.slice("app".length))}]=${JSON.stringify(json)}`);
            if (pagePath === "app/not-found") {
                // Create a separate special manifest for the root not-found page.
                assets["server/app/_not-found_" + CLIENT_REFERENCE_MANIFEST + ".js"] = new sources.RawSource(`globalThis.__RSC_MANIFEST=(globalThis.__RSC_MANIFEST||{});globalThis.__RSC_MANIFEST[${JSON.stringify("/_not-found")}]=${JSON.stringify(json)}`);
            }
        }
